...
// Format no. telefon ke bentuk chatId Green API (nombor@c.us).
...
// Hantar mesej WhatsApp ke MANA-MANA nombor (bukan admin sahaja) - guna
// kelayakan Green API yang sama. Dipakai untuk peringatan kepada pelanggan.
export async function sendWhatsapp(phone, text) {
  const { GREEN_API_URL, GREEN_API_ID_INSTANCE, GREEN_API_TOKEN_INSTANCE } = process.env;
  if (!GREEN_API_URL || !GREEN_API_ID_INSTANCE || !GREEN_API_TOKEN_INSTANCE) {
    console.warn("[notify] Pembolehubah Green API belum lengkap - WhatsApp dilangkau.");
    return { skipped: true };
  }
  const url = `${GREEN_API_URL}/waInstance${GREEN_API_ID_INSTANCE}/sendMessage/${GREEN_API_TOKEN_INSTANCE}`;
  try {
    const res = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ chatId: toWhatsappChatId(phone), message: text }),
    });
    const body = await res.text();
    return { sent: res.ok, body };
  } catch (err) {
    console.error("[notify] sendWhatsapp gagal:", err);
    return { sent: false, error: String(err) };
  }
}
